...
业务命令也会在连接数据库前自动执行迁移。下载、分析和发布均只处理过去 30 天内已发布的视频，
并优先处理发布日期较新的候选；发布日期相同的下载任务先执行待下载任务，再重试历史失败任务。
下载失败只影响当前视频，本轮继续处理队列。
...
当前 analysis prompt v3 和 `schema_version = "2"` 重新分析。升级前已存在且仍处于 30 天窗口内
的未完成 outbox 由 `push` 恢复，不受首次入队条件限制。

`015_prioritize_video_publication_date.sql` 重建发布日期和字幕下载队列索引。Pipeline 候选查询使
下载、分析和发布均只处理过去 30 天内已发布的视频并按发布日期倒序消费；发布日期缺失、发布
超过 30 天或日期在未来的记录不进入队列。
...
普通调度从最新视频扫描到运行前已知 video ID 即停止，`0`、`1`、`2` 都是边界，30 天窗口内的
状态 `2` 仍由下载队列重试。`--force` 保持完整扫描或服从显式数量限制，但同样不能绕过 30 天
窗口。旧库中已有视频但首次
