...
业务命令也会在连接数据库前自动执行迁移。下载、分析和发布均优先处理发布日期较新的视频；
发布日期相同的下载任务先执行待下载任务，再重试历史失败任务。下载失败只影响当前视频，本轮继续处理队列。
...
`015_prioritize_video_publication_date.sql` 重建发布日期和字幕下载队列索引，使下载、分析和
发布均按视频发布日期倒序消费；缺失发布日期的历史记录排在最后。
